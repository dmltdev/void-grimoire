...
description: Use when the user wants to control response length, depth, or token budget — offers an informed choice about how much depth to consume before answering. Triggers: explicit asks about token budget/count/usage/limit, response length, answer depth, short vs detailed, "brief answer", "exhaustive answer", or equivalent phrasings in any language. Do not trigger when a level was already set in this session, when the request needs a one-word answer, or when "token" means auth/session/payment.
depends-on: []
chains-to: null
suggests: []
---
