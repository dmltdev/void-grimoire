...
3. **Install target:** recommended default = current repo's existing skill directory and catalog conventions.
4. **Verification:** recommended default = metadata parse + source-of-truth/link checks + one pressure scenario.
...
- Capture exact failures: invalid metadata, vague trigger, skipped tests, fake verification, bloated prose, invented or missed catalog metadata, wrong install path, scope creep.
...
- Skill catalog metadata or manifest, if present.
...
### 4. Choose the canonical name

Name the skill for the trigger or action a future agent will search for. Apply the first matching row:

| Observable condition | Naming pattern | Examples |
|---|---|---|
| An established repository family already fits | Reuse that family | `git-push`, `session-summary`, `audio-plan` |
| The skill applies or operates a tool, platform, mode, style, or protocol | `using-<subject>` | `using-codex`, `using-orx`, `using-simple-english` |
| The skill performs bounded work that creates or changes something | `<action>-<object>` | `document-ideas`, `expand-prompt`, `index-docs` |

- Use lowercase kebab-case with 1-64 characters. Start and end with a letter or number.
- Keep the directory, frontmatter `name`, and every existing catalog or README name identical.
- Prefer `using-*` over imperative `use-*`.
- For a rename, perform one clean cutover. Remove the old name and do not leave an alias.


### 5. Choose the narrow contract
...
### 6. Write frontmatter for discovery
...
- `name` matches the directory and any existing catalog entry.
- Description starts with `Use when`.
- Description is trigger-only. Do not summarize the workflow.
- Include concrete trigger terms from the target skill's domain. For skill-authoring skills, useful terms include `SKILL.md`, plugin, slash command, catalog, prompt, verifier, TDD, pressure scenario, package, and install.
- Keep marketing out.

### 7. Write the body as an execution contract
...
- Prefer clean cutover: one canonical skill name, no aliases or duplicate skill or catalog entries unless explicitly approved.
...
### 8. Install in one clean cutover

For a plugin skill addition, update every source of truth that lists skills:

- `skills/<category-name>/<skill-name>/SKILL.md`, when the plugin groups skills by category.
- Existing skill registry or catalog metadata, if the target plugin uses one.
...
| Name consistency | Directory, frontmatter, and every existing catalog or README use the same name. |
...
---
name: skill-forge
description: Use when creating, revising, auditing, testing, or installing agent skills, SKILL.md files, plugin skills, prompt-quality guidance, or catalog-backed skill additions.
---
```

The body must then require local convention lookup, pressure scenarios, exact output shape, updates to repository-owned catalogs or docs, and validation evidence.
...
| Verification is unclear | Define metadata, source-of-truth, link, placeholder, and pressure-scenario checks. |
...
| Updating `SKILL.md` but not the repository's existing catalogs or README. | Install in one clean cutover. |
