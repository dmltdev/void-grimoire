...
- Canonical plugin skills live under `skills/<category-name>/<skill-name>/SKILL.md`.
- The categorized `skills/` directory tree is the single category map, and the README catalog mirrors it; neither controls runtime routing.
- `.agents/skills/void-install-skills` owns local reinstallation. `.claude/skills` points to `.agents/skills`.
- One capability has one canonical skill name. Renames and replacements use a clean cutover, without aliases or duplicate skill definitions.
...
Catalog impact: skill tree and README updates required or not required
...
| Add, revise, rename, or remove a shipped skill | `skills/<category-name>/<skill-name>/SKILL.md` | README catalog/count when membership changes, version manifests, local reinstall |
| Change a skill category | Move its canonical directory under `skills/<new-category-name>/` | README domain table; update the Claude manifest when adding or removing a category |
...
### 4. Keep the filesystem and catalog coherent

When skill membership changes:

1. Place the canonical skill directory under exactly one category in `skills/`.
2. Update the README's total count, domain row, and relevant headline entry.
3. For a rename, remove every old README catalog name and old skill directory in the same cutover.
4. Do not retain aliases, forwarding skills, duplicate definitions, or compatibility copies unless the user explicitly approves that exception.

A content-only edit to an existing skill does not require a README catalog rewrite.
...
- Each canonical name exists exactly once under `skills/*/*/SKILL.md`, and the README domain table contains it exactly once; intentional headline or prose mentions are allowed.
- README count equals the number of canonical `skills/*/*/SKILL.md` files.
