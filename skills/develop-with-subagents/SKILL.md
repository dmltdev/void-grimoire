...
digraph when_to_use {
    ...
    "develop-with-subagents" [shape=box];
    ...
    "Stay in this session?" -> "develop-with-subagents" [label="yes"];
    "Stay in this session?" -> "execute-plan" [label="no - parallel session"];
}
```
...
digraph process {
    ...
    "Dispatch final code reviewer subagent for entire implementation" -> "Use finish-branch";
}
```
...
- **develop-tdd** - Subagents follow TDD for each task
