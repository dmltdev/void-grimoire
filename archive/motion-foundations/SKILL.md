...
domain: patterns
description: Use when establishing Framer Motion (motion/react) primitives in a React/Next.js codebase — motion tokens, spring presets, perf rules, reduced-motion accessibility, SSR safety. Foundation for the other motion-* code skills (not the impeccable design-animate workflow).
depends-on: []
chains-to: null
suggests: []
---
...
> Canonical reference: [Motion (Framer Motion) docs](https://motion.dev/docs). This skill captures battle-tested patterns; defer to the upstream docs for current API.
